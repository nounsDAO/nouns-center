...
const isDark = () =>
  ...
  (!('theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches); //Condition 2 - No theme key in local storage but media color scheme is dark
...
const ThemeSwitch = () => {
  ...
  const darkModeActive = process.browser && document.documentElement.classList.contains('dark'); // returns true if its a client and 'dark' is present in html
      ...
      >
        ...
        <span
          ...
          className={classNames(
            darkModeActive ? 'bg-gray-500 focus:ring-gray-500' : 'bg-white focus:ring-white',
